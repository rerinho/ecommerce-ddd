...
import { INVALID_UUID_MESSAGE } from "~/shared/guard-clauses/clausules/IsUuid";
  ...
  describe("should throw error, when the entered value", () => {
    describe("its not a valid uuid", () => {
      test.each([
        ...
        expect(() => Uuid.Create(value)).toThrowError(
          Error(INVALID_UUID_MESSAGE("uuid"))
        );
      });
    });
  });

  describe("should create a Uuid instance when a valid value is entered", () => {
    test.each([
